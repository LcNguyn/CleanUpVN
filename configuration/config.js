module.exports = {
  /*
   * This file contains the configurations information of Twitter login app.
   * It consists of Twitter app information, database information.
   */

  facebook_api_key: "483735179161974",
  facebook_api_secret: "6d85c1f4acae9b2af0b31419440c0700",
  callback_url: "http://localhost:3000/sociallogin/auth/facebook/callback",
  use_database: true,
  host: "localhost",
  username: "root",
  password: "locloc123",
  database: "sakila",
  awsAccessKey: 'ha5Wj/igdg/BCqsVydnJMRajiwD/Tvq5mjJPzlqt',
  awsAccountId: 'AKIAXOKDVMKSJEOIRZF7'
};
